...
export async function spawnHostService(
	...
	if (!existsSync(hostBin)) {
		if (isDesktopBundled()) {
			throw new Error(
				"`superset start` cannot launch a new host service from the CLI bundled with Superset Desktop; Desktop runs the host service itself. For headless use, install the standalone CLI: curl -fsSL https://superset.sh/cli/install.sh | sh",
			);
		}
		throw new Error(
