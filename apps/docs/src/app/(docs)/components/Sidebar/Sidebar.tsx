		...
		<div className={cn("fixed start-0 top-0")}>
			...
			>
				<div className="shrink-0 px-4 pt-4 pb-2">
					<button
						type="button"
						className="flex w-full items-center gap-2 rounded-md border border-border/50 px-2.5 py-1.5 text-sm text-muted-foreground/80 transition-colors hover:border-border hover:text-foreground"
						onClick={() => setOpenSearch(true)}
					>
						<Search className="size-3.5 shrink-0 opacity-70" />
						<span className="grow truncate text-left">Search docs...</span>
						<kbd className="text-[11px] font-medium text-muted-foreground/60">
							⌘K
						</kbd>
					</button>
				</div>
				...
				>
					{sections.map((section) => (
						<div key={section.title}>
							<p className="px-2 pb-1.5 text-[11px] font-semibold uppercase tracking-wider text-brand dark:text-brand-light">
								{section.title}
							</p>
