...
<Command
	...
	options={[
		{ flag: "--workspace <id>", required: true, description: "Workspace UUID whose terminals to list." },
		{ flag: "--host <id>", description: "Host the workspace lives on (default: this machine)." },
	]}
	output={`{
  sessions: Array<{
...
```bash
superset terminals list --workspace ws_…
```
</Command>
...
<Command
	...
	options={[
		{ flag: "--workspace <id>", required: true, description: "Workspace UUID the terminal runs in." },
		{ flag: "--host <id>", description: "Host the workspace lives on (default: this machine)." },
		{ flag: "--terminal <id>", required: true, description: "Terminal ID (the sessionId `agents create` returned)." },
		{ flag: "--text <text>", required: true, description: "Text to write into the terminal." },
...
>
Send a follow-up message to a terminal already running in a workspace — e.g.
a follow-up prompt to a claude/codex agent — instead of spawning a new
session. Multi-line text is delivered as a single paste.

Prefer `agents create` once to start an agent, then `terminals send` for
every follow-up: re-running `agents create` per message spawns a new
session instead of continuing the conversation.

```bash
superset terminals send --workspace ws_… --terminal <sessionId> --text "Also add a test for the empty case"
```
</Command>
...
<Command
	...
	options={[
		{ flag: "--workspace <id>", required: true, description: "Workspace UUID the terminal runs in." },
		{ flag: "--host <id>", description: "Host the workspace lives on (default: this machine)." },
		{ flag: "--terminal <id>", required: true, description: "Terminal ID to read." },
		{ flag: "--max-lines <n>", description: "Cap returned rows from the bottom." },
	]}
	output={`{
  terminalId: string;
  cols: number;
...
```bash
superset terminals read --workspace ws_… --terminal <sessionId>
```
</Command>
...
<Command
	...
	options={[
		{ flag: "--workspace <id>", required: true, description: "Workspace UUID the terminal runs in." },
		{ flag: "--host <id>", description: "Host the workspace lives on (default: this machine)." },
		{ flag: "--terminal <id>", required: true, description: "Terminal ID to close." },
	]}
	output={`{
  terminalId: string;
  status: string;
...
```bash
superset terminals close --workspace ws_… --terminal <sessionId>
```
</Command>
