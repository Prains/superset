...
<Command
	...
	options={[
		...
		{ flag: "--port <n>", description: "Specific port. Default: a free port." },
		{ flag: "--host <addr>", description: "Address to bind. Default: all interfaces. Pass 127.0.0.1 to restrict to localhost." },
...
By default the host server binds the wildcard address, so it is reachable
on every interface of the machine — including LAN, VPN, and tailnet
interfaces. Requests still need the per-start pre-shared key from the
`0600` manifest, but if you only ever reach it from the same machine,
pin it:

```bash
superset start --daemon --host 127.0.0.1
```

The startup log prints the address it actually bound, so you can confirm
which it is.
