...
Settings changes are picked up by a running desktop app the next time its
window regains focus. Theme changes require an app restart (quit the app
first, or it may overwrite the change). Git settings (`branchPrefixMode`,
`branchPrefixCustom`, `worktreeBaseDir`) are host-wide and are written
through the local host service, so the desktop app or `superset start`
must be running for those.
