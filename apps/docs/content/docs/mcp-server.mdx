...
| **Terminals** | list, create, send follow-up text to a running terminal, read its screen back, close |
...
| `terminals_list` | List the live terminal sessions in a workspace |
| `terminals_send` | Send follow-up text to a terminal already running in a workspace (e.g. a claude/codex agent), instead of spawning a new session |
| `terminals_read` | Read a terminal's current screen and recent scrollback back as plain text |
| `terminals_close` | Close (dispose) a terminal running in a workspace |

Prefer `agents_create` once to start an agent, then `terminals_send` for every follow-up message and `terminals_read` to observe the reply — re-running `agents_create` per message spawns a new session instead of continuing the conversation.
