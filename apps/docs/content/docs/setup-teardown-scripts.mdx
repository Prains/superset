...
New worktrees get a copy of the main repo's `.superset` directory so gitignored scripts still resolve. Credential files are left out of that copy: dotenv files (`.env`, `.env.local`, …), `credentials.json`, `.netrc`, and `.pem`/`.key`/`.p12`/`.pfx` files. The copy happens once, at worktree creation, and is never refreshed — a snapshotted API key would keep serving its old value in that worktree long after you rotated it.

Read secrets from the main checkout at run time instead, so every workspace resolves the current value:
