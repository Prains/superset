...
Closing a port (or **Close all ports** for a workspace) shows a confirmation before killing the underlying process, the same destructive-action prompt used for closing terminals. Cancel or dismiss leaves the processes running. Enable **Don't ask again** to skip the prompt going forward; it shares the same preference as the running-process confirmation for terminals.
