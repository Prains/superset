...
Nothing outstanding — the four Open Questions below (threshold value, settings-page location, v2 call site, copy) were resolved during implementation; the initial threshold (5) and copy remain placeholders pending real product/design review, not implementation blockers.
...
Each command should be run from the `apps/desktop` directory unless otherwise noted; `bun run lint`/`bun run lint:fix` are actually root-level commands per `AGENTS.md` ("Biome runs at root level, not per-package") — run those specifically from the repository root instead:
    ...
    cd /Users/avipeltz/.superset/worktrees/1c99c8eb-1b31-4f04-9ac4-61a2760c74b6/political-magnosaurus
    bun run lint:fix
    bun run lint
