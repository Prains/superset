...
The desktop hydrates a persisted token into an in-memory bearer-token closure. A raw `Runtime.evaluate` sign-in `fetch` can create a cookie session, but it does not populate or persist the bearer token used by the Electron host-service coordinator. For a real Electron E2E journey, prefer the app-owned local-dev sign-in flow: navigate the matched renderer to `#/sign-in` and click **Sign in as Local Admin (dev)** through CDP `Input.dispatchMouseEvent`. That flow signs in, calls `electronTrpc.auth.persistToken`, updates the in-memory auth client, and navigates to the workspace UI. Do not replace this click with a raw sign-in request when the test needs host-backed features such as projects, workspaces, terminals, or agents.

After the app-owned sign-in, verify `/api/auth/get-session` from the matched renderer with `credentials: "include"` and require both `session` and `session.activeOrganizationId`. If diagnosing bearer hydration, import `/lib/auth-client.ts` and return only a boolean such as `Boolean(getAuthToken())`; never return or print the token itself.

If a test only needs a cookie-authenticated API probe and does not need the Electron host service, a workspace created by `setup.local.sh` can repair its CDP cookie session as follows:
...
This credentialed local-dev sign-in creates the browser session cookie needed by subsequent in-renderer fetches, but it is not a substitute for the app-owned sign-in when host-backed behavior is under test. If it fails, report the sign-in/session status codes only. For a non-local setup, use the app's normal sign-in flow; never substitute local dev credentials.
