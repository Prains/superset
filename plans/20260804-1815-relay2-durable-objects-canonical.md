...
- Observation: The web app's CSP derives `connect-src` from its own `RELAY_URL` build env (`apps/web/next.config.ts`), while its runtime relay URL comes from the `relay-url-override` flag — so pointing a user at a new relay via the flag silently breaks every web client until the web deployment's env is updated too. Same class as the desktop CSP gap, and it makes web-as-client unusable for testing a new relay. A rollout prerequisite, not a code bug.
  Evidence: `connect-src` list is built from `relayHttpOrigin`/`relayWsOrigin`/`relayBackup*` only; no allowance for a flag-supplied origin.
- Observation: With `FORCE_RELAY_ROUTING` removed, the desktop routes its own machine's workspaces straight to 127.0.0.1, so a single machine running the canary cannot exercise a relay at all — testing requires a second host that speaks tunnel v2. Released host-services speak v1 until a stable release carries the v2 client (desktop/host-service/cli ship in lockstep), so any test host must run host-service from source until then.
